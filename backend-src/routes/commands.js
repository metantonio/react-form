...
router.post('/unix-commands', (req, res) => {
    ...
    if (platform === 'win32') {
        ...
        try {
            ...
            if ((data == 'pwd' && lesson < 4)) {
            ...
            else if (commandParts[0] === 'cd' && lesson == 3) {
                let resto = commandParts.slice(1)
                childProcess = spawn('pwd', resto, { shell: true, cwd: "./unix" });
                ...
                childProcess = spawn('pwd', [commandParts[1]], { shell: true, cwd: "./unix" });
            }
            ...
            else if (commandParts[0] === 'find') {
                let resto = commandParts.slice(1)
                childProcess = spawn('find', resto, { shell: true, cwd: "./unix/home/user1" });
                if (data == 'find ./ -type f -name "*.txt" | grep "secret"' || data == 'find . -type f -name "*.txt" | grep "secret"' && (lesson == 8 || lesson == 11)) {
                    correcto = true
                } else {
                    correcto = false
                }
